...
import { getAIPoweredExercises } from "@/app/actions/game";
...
const Results = ({
  ...
  const [aiExercises, setAiExercises] =
    useState<PersonalizedExercisesOutput | null>(null);
    ...
    try {
      ...
      if (stats.errors.size > 0) {
        const errorEntries = Array.from(stats.errors.entries())
          .sort((a, b) => b[1] - a[1])
          .slice(0, 5);
        const commonErrors = errorEntries
          .map(
            ([char, count]) =>
              `'${char === " " ? "space" : char}' (${count} times)`
          )
          .join(", ");
