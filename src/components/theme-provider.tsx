...
const CustomThemeContext = React.createContext<
  CustomThemeProviderState | undefined
>(undefined);
...
export function ThemeProvider({
  ...
  const [colorTheme, setColorTheme] = React.useState<ColorTheme>(() => {
    ...
    return (
      (localStorage.getItem(COLOR_THEME_KEY) as ColorTheme) || "theme-blue"
    );
  });
  ...
  return (
    ...
    >
      <InnerThemeProvider
        setColorThemeOuter={setColorTheme}
        colorThemeOuter={colorTheme}
      >
        {children}
      </InnerThemeProvider>
    </NextThemesProvider>
...
function InnerThemeProvider({
  ...
  children: React.ReactNode;
  setColorThemeOuter: (theme: ColorTheme) => void;
  colorThemeOuter: ColorTheme;
}) {
  ...
  const value = React.useMemo(
    ...
    [theme, setTheme, colorThemeOuter, setColorThemeOuter]
  );
