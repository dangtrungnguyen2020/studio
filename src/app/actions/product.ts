...
async function translateProduct(product: Product, targetLocale: string): Promise<Product> {
    ...
    const needsTranslation = (field: keyof Product) => !(product[field] as any)[targetLocale];
    ...
    if (needsTranslation('name')) {
        translationsToPerform.push(
            translateText({ text: product.name.en, targetLanguage: targetLocale })
            .then(res => ({ field: 'name', value: res.translation }))
        );
    }
    if (needsTranslation('description')) {
        translationsToPerform.push(
            translateText({ text: product.description.en, targetLanguage: targetLocale })
            .then(res => ({ field: 'description', value: res.translation }))
        );
    }
    if (needsTranslation('features')) {
        translationsToPerform.push(
            Promise.all(product.features.en.map(feature => translateText({ text: feature, targetLanguage: targetLocale })))
            .then(res => ({ field: 'features', value: res.map(f => f.translation) }))
        );
    }
    if (needsTranslation('specs')) {
        ...
        translationsToPerform.push(
            Promise.all(specValues.map(value => translateText({ text: value, targetLanguage: targetLocale })))
    ...
    if (Object.keys(fieldsToUpdate).length > 0) {
        updateDoc(doc(db, 'products', product.id), fieldsToUpdate).catch(console.error);
    }
